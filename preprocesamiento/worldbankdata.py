...
def get_indicator_data_country(country, indicator, dates):
    response = iterate_by_page(sub_query = '/'+country+'/indicator/'+indicator, params_dict = {'date':dates})
...
if __name__ == '__main__':
    ...
    for indicator in indicators:
        ...
        for country in countrys:
            ...
            try:
                df = get_indicator_data_country(country, indicator["indicator_code"], dates)
        ...
        df_final = pd.concat(df_list)
        df_final.to_csv('World_Bank_Data/'+indicator["indicator_code"]+'_'+dates.replace(':', '-')+'2.csv', index=False, encoding='utf-8')
