...
def merge_links(df, by_columns = ['Exporter','Importer'],HarmonizedSystem = 4):
    ...
    Args:
        df (pd.DataFrame)
        col_category (str): Column to group by
        col_value (str): Column to sum
...
def add_metadata(df_left, df_right, tuple_columns):
    """Add metadata to the columns of a dataframe.
    
    Args:
        df_left (pd.DataFrame): Dataframe to add metadata to.
        df_right (pd.DataFrame): Dataframe containing metadata.
        tuple_columns (set): array with 2 elements (column_name, metadata_column_name)
    Returns:
        pd.DataFrame: Dataframe with metadata.
    """
    #df_right = df_right.rename(mapper=lambda x : col_metadata + str(x), axis=1)
    #return df_left
    df_right = df_right.rename(mapper=lambda x : tuple_columns[0]+'_' + str(x), axis=1)
    df_result = df_left.merge(df_right, left_on=tuple_columns[0], right_on=tuple_columns[0]+'_'+tuple_columns[1], how='inner')
    return df_result
...
def set_node_atrributes(G, df, column_name):
    for node in G.nodes:
        G.nodes[node]['name'] = df[df['iso3'] == node][column_name].values[0]
    return G
...
class BACI_Cleaner:
    ...
    def __init__(self, dataset):
        ...
        self.product_codes_filename = f'product_codes_{self.HS_YEAR}_{self.VERSION_DATASET}.csv'
        self.country_codes_filename = f'country_codes_{self.VERSION_DATASET}.csv'
        
        self.df_product_codes = pd.read_csv(self.FOLDERS['raw'] +'/'+ self.DATASET + '/' + self.product_codes_filename, dtype='string')
        self.df_country_codes = pd.read_csv(self.FOLDERS['raw'] +'/'+ self.DATASET + '/' + self.country_codes_filename, dtype='string' , encoding='latin-1')
        ...
        self.df_gdp_linked = pd.read_csv('World_Bank_Data/NY.GDP.DEFL.ZS.AD_1995-20222.csv', dtype='string')
        
        
    def get_gdp_deflator(self, year, country='USA'):
        # Obtiene el deflactor de gdp para un año
        
        bool_mask = (self.df_gdp_deflator['countryiso3code'] == country) & (self.df_gdp_deflator['date'] == str(year))
        df_gdp_deflator = self.df_gdp_deflator[bool_mask]
        ...
        if df_gdp_deflator.empty:
            ...
            gdp_deflator_decimal = gdp_deflator/100
            return gdp_deflator_decimal
    
    def get_cummulative_gdp_deflator(self, years:set()):
        ...
        inflation_list = [(1+x) for x in gdp_deflator_list]
    ...
    def get_gdp_linked(self, year, country='USA'):
        ...
        bool_mask = (self.df_gdp_linked['countryiso3code'] == country) & (self.df_gdp_linked['date'] == str(year))
    ...
    def to_constant_USD(self, df, year):
        ...
        df['Value'] = df['Value'].apply(lambda x: float(x)/float(gdp_linked)*100)
        return df
    ...
    def data_clean_by_year(self, filename, money_type='current_USD', graph=True):
        ...
        raw_file = filename
        raw_df = pd.read_csv(self.FOLDERS['raw'] +'/'+ self.DATASET + "/" + raw_file, dtype='string')
        ...
        raw_df['Product category HS[2]'] = raw_df['Product category'].apply(lambda x: x[:2])
        raw_df['Product category HS[4]'] = raw_df['Product category'].apply(lambda x: x[:4])
        
        clean_df = raw_df[['Year', 'Exporter_iso_3digit_alpha', 'Importer_iso_3digit_alpha', 'Value', 'Quantity',
        'Product category_code']].copy()
        
        clean_df.rename(columns={"Exporter_iso_3digit_alpha": "Exporter", "Importer_iso_3digit_alpha": "Importer"}, inplace=True)
        ...
        if graph == True:
            ...
            for indicator in indicators:
                df_ind = pd.read_csv("World_Bank_Data"+"/"+indicator["indicator_code"]+"_1995-2022.csv", dtype='string')
                year = year_file[1:]
                df_ind.query('date == @year', inplace=True)
                indicator_dict = df_ind[['countryiso3code', 'value']].dropna().to_dict(orient="records")
                indicator_dict = {record['countryiso3code']: record['value'] for record in indicator_dict}
            ...
            nodes_sin_propension = [n for n, attr in G.nodes(data=True) if 'propension' not in attr or 'ingreso' not in attr]
        ...
        #nx.write_graphml(G, SAVE_FOLDER+'/'+'Graph_'+filename.replace('.csv','.graphml'))
